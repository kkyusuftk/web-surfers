...
// Jump animation curve parameters
const JUMP_GRAVITY = -9.8;
const INITIAL_JUMP_VELOCITY = Math.sqrt(-2 * JUMP_GRAVITY * JUMP_HEIGHT);
...
export const Player = () => {
  ...
  const [ref, api] = useBox(() => ({
    ...
    onCollide: (e) => {
      if (e.body.userData?.type === 'barrier' || e.body.userData?.type === 'car') {
        endGame();
      }
    }
  }));
  ...
  useEffect(() => {
    ...
    if (isRolling && !isRollingRef.current && !isJumpingRef.current && canRoll.current) {
  ...
  useFrame((_, delta) => {
    ...
    const newX = THREE.MathUtils.lerp(currentX, targetLanePosition.current, delta * 10);
    ...
    if (isJumpingRef.current) {
      ...
      if (playerModelRef.current) {
        ...
        if (jumpProgress < 0.3) {
          ...
          playerModelRef.current.children[1].rotation.x = 0.5 - (extendProgress * 1.3);
          playerModelRef.current.children[2].rotation.x = 0.5 - (extendProgress * 1.3);
          ...
          playerModelRef.current.children[1].rotation.x = -0.4 + (landProgress * 0.4);
          playerModelRef.current.children[2].rotation.x = -0.4 + (landProgress * 0.4);
        }
