...
export const Car = ({ position, color = '#3498db' }: CarProps) => {
  ...
  useEffect(() => {
    const unsubscribe = api.position.subscribe(p => (carPosition.current = p));
    return unsubscribe;
  }, [api]);
  ...
  return (
    <group ref={ref as any}>
      ...
      <mesh castShadow position={[0, carHeight * 0.5, -0.2]}>
        <boxGeometry args={[carWidth * 0.8, carHeight * 0.4, carLength * 0.6]} />
        <meshStandardMaterial color={color} />
      </mesh>
      
      {/* Wheels */}
      <mesh castShadow position={[carWidth/2 * 0.8, -carHeight/2 + 0.2, carLength/3]} rotation={[Math.PI/2, 0, 0]}>
